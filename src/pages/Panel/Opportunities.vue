<template>
    <div>
      ...
      >
        ...
        <div class="create-button" @click="createNewOpportunity">
      ...
      >
        ...
        <template v-slot:[`item.action_opportunity`]="{ item }">
          <div class="delete-img-wrapper">
            <svg style="margin-right: 10px" @click.stop="startDeletionData(item.id)" width="13" height="16" viewBox="0 0 13 16" fill="none"
            ...
            <button 
              class="small-button"
              @click.stop="$router.push(`/panel/opportunities/${item.id}/alerts`)"
            >
              show alerts
            </button>
          </div>
      ...
      <modal-window v-if="isDeleteModalOpened" @close="closeDeletion">
        <template v-slot:title>
            ...
            {{ selected.length > 1 ? 'these opportunities?' : 'this opportunity?' }}
        </template>
            ...
            <div class="big-modal-window">
              <div class="left-side">
                ...
                <div class="bottom-block">
                  <label for="rate">
                    Rate
                    <Field type="text" name="rate" id="rate" />
              ...
              <div class="right-side">
                ...
                <div class="attach-files">
                  ...
                  <!-- <span v-else class="file-error">
                    {{ attachedFileName.msg }}
                  </span> -->
                </div>
...
<script>
    ...
    const $updateOneDataRequest = (id, data) => $api.OpportunitiesRequest.update(id, data);
    const $createOneDataRequest = (data) => $api.OpportunitiesRequest.create(data);
    const $getRequest = (query) => $api.OpportunitiesRequest.getAll(query);
    const $deleteOneDataRequest = (id) => $api.OpportunitiesRequest.deleteSelected([id]);
    ...
    const onSubmit = (router, configData, configAction, initialValues) => {
        return async (data) => {
          ...
          if (!!id) {
            ...
            formData.append('attachmentLinksToRemove', '');
          ...
          if(result) {
            await configData.getData();
            configAction.closeOpportunityModal()
          }
        }
    };
    ...
    export default {
        ...
        setup() {
            ...
            const onRowClick = event => {
              ...
              $configAction.createOpportunity(event.id)
            }
          ...
          const $configData = pageDataWorkerSetup({
              $initData: {
                reactiveList: route.meta.opportunitiesData.data.items.map(transformOperatorItems),
                reactiveFilterFields: {
                    ...filterFields,
                    ...route.query
                },
                reactivePagination: $getPagination(route.meta.opportunitiesData),
                formData: []
              },
          ...
          const switchStatus = async (id, status) => {
            ...
            await $api.OpportunitiesRequest.patch(id, data);
            await $configData.getData();
          }
          ...
          return {
              ...
              cancelRating,
              arrayOfStatusObject,
              onSubmit: onSubmit(router, $configData, $configAction, initialValues ),
              schema,
              envAddress,
